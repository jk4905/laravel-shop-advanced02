...
class CategoriesController extends Controller
{
    ...
    protected function form($isEditing = false)
    {
        ...
        if ($isEditing) {
            ...
            // 定义一个名为『是否目录』的单选框
            $form->radio('is_directory', '是否目录')->options(['0' => '否', '1' => '是'])->default(0)->rules('required');
            // 定义一个名为父类目的下拉框
    ...
    public function apiIndex(Request $request)
    {
        ...
        $result = Category::query()->where('is_directory', boolval($request->input('is_directory')))  // 由于这里选择的是父类目，因此需要限定 is_directory 为 true
